...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='bc4763cbf8b9e6adfbc07bb3194509b187b0b901'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'refs/pull/115500/head, release-1.22' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'refs/pull/115500/head, release-1.22' =~ tag:\ (v[^ ,]+) ]]; then
